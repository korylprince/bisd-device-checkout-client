<template>
  <form
    novalidate
    @submit.prevent="do_checkout_device(other_id, bag_tag, note)"
  >
    <md-card>
      <md-card-header>
        <div class="md-title">
      ...
      <md-card-content>
        ...
        <div v-if="devices && devices.length > 0">
          ...
          <span v-for="(device, index) in devices" :key="device.link">
            <a style="padding: 10px" :href="device.link" target="_blank"
              >Device {{ index + 1 }}</a
            >
          </span>
        </div>
        ...
        <div v-if="charges && charges.length > 0">
          ...
          <span v-for="(charge, index) in charges" :key="charge.link">
            <a style="padding: 10px" :href="charge.link" target="_blank"
              >Charge {{ index + 1 }} - ${{ charge.link_value }}</a
            >
          </span>
        </div>

        <md-field
          :class="{ 'md-invalid': errors.has('bag tag') }"
          v-if="status && status['type'] !== 'none'"
        >
          <label>Bag Tag</label>
          <md-input
...
export default {
    ...
    computed: {
        ...
        student() {
            return this.students[
                this.students.findIndex(student => student.other_id === this.other_id)
            ]
        },
        reason() {
            if (
                this.status == null ||
        this.status.issues == null ||
        ...
        devices() {
            if (this.status == null || this.status.issues == null) {
                return null
            }
    ...
    methods: {
        ...
        async do_checkout_device(other_id, bag_tag, note) {
            if (
                bag_tag.match(/^\d{4}$/) == null ||
        (this.status && this.status["type"] === "none")
            ) {
                return
            }
